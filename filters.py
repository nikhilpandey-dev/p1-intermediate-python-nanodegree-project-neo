...
method `get` that subclasses can override to fetch an
...
class AttributeFilter:
    ...
    def __init__(self, op, value):
        ...
        The new AttributeFilter is created from an binary
         predicate & a reference value. The reference value will
          be supplied as the second (right-hand side)
...
class DateFilter(AttributeFilter):
    ...
    Description:
    It override the `get` method of its superclass to
     obtain filter on dates.
    """
...
class DistanceFilter(AttributeFilter):
    ...
    Description:
    It override the `get` method of its superclass
     to obtain filter on distance.
    """
...
class VelocityFilter(AttributeFilter):
    ...
    Description:
    This subclass overrides the `get` method of its superclass
     to obtain filter on velocity.
    """
...
class DiameterFilter(AttributeFilter):
    ...
    Description:
    This subclass of AttributeFilter overrides the `get` method
...
class HazardousFilter(AttributeFilter):
    ...
    Description:
    This subclass of AttributeFilter overrides the `get` method
     of its superclass to obtain information about
      hazardous state of the neo of the CloseApproach.
    """
    ...
    argument, and represents a filter that selects close
     approaches that occured on exactly that given date.
      Similarly, the `--min-distance` option corresponds to the
       `distance_min` argument, and represents a filter that
        selects close approaches whose nominal approach distance
         is at least that far away from Earth. Each option is `None`
          if not specified at the command line (in particular,
           this means that the `--not-hazardous` flag results in
            hazardous=False`, not to be confused with `hazardous=None`).

    The return value must be compatible with the `query`
     method of `NEODatabase` because the main module directly passes
