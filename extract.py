...
def load_neos(neo_csv_path='./data/neos.csv'):
    ...
    with open(neo_csv_path, 'r') as infile:
        ...
        for e in reader:
            neo = NearEarthObject(e['pdes'], e['name'], e['diameter'], e['pha'])
...
def load_approaches(cad_json_path='./data/cad.json'):
    ...
    with open(cad_json_path, 'r') as infile:
        ...
        json_data = [dict(zip(contents["fields"], data)) for data in contents["data"]]
    
    for data in json_data:
        close_approach = CloseApproach(data["des"], data["cd"], data["dist"], data["v_rel"])
